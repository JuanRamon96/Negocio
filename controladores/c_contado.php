...
class contado {
	...
	public function _insertar(){
		...
		extract($_POST);
		$query = ("insert into ventas (id_venta,cliente,usuario,fecha,total,descuento,folio,estatus,dias_pagar,iva,sucursal,tipo_venta) VALUES 
		(null, '$cliente', '".$_SESSION['id_tel']."', '$fecha', '$total', '$descuentoDine', '$folio', '$estatus', '$diaspPagar', '$ivaVen', '$sucursal', '$tventa');");
		$error = $omodelo->_insertar($query);
		
		$query1 = ("SELECT * FROM ventas order by id_venta desc limit 1;");	// ::::::::::::::: MODIFICAR EL QUERY
		...
		for($y=0;$y<count($detalle);$y++){
			...
			$query2 = ("INSERT INTO detalle_ventas (id_detalle,venta,producto,cantidad,subtotal)
			values(null,'".$row1[0]['id_venta']."','".$detalleB[0]."','".$detalleB[1]."','".$detalleB[2]."');");
			$row2 = $omodelo->_insertar($query2);

			$query7 = ("SELECT SUM(cantidad) as cantidad FROM detalle_productos where producto = '".$detalleB[0]."';");
			$row7 = $omodelo->_consultar($query7);
			$query8 = ("SELECT * FROM productos where id_producto = '".$detalleB[0]."';");
			$row8 = $omodelo->_consultar($query8);
			
			$elCosto = (double)$row8[0]['costo'] / (double)$row7[0]['cantidad'];
			$elCosto = (double)$detalleB[1] * $elCosto;
			
			$querys = ("UPDATE productos set costo=costo-".$elCosto." where id_producto = '".$detalleB[0]."';");
			$rows = $omodelo->_insertar($querys);
// ------------------------------------------------------------------------------------------------------------------------------------------------------------
			// AQUI SE MODIFICA LA EXISTENCIA DEL PRODUCTO EN LA TABLA DETALLE DE PRODUCTOS 
			$query6 = ("UPDATE detalle_productos set cantidad=cantidad-'".(double)$detalleB[1]."' where producto = '".$detalleB[0]."' and sucursal='".$sucursal."';");
			$row6 = $omodelo->_insertar($query6);
		}
		if($tventa != 'Contado'){
			$query4 = ("SELECT * FROM clientes where id_cliente = '".$cliente."';");
			$row4 = $omodelo->_consultar($query4);

			$diasb = (int)$diaspPagar - 5;
			...
			(null, '".$_SESSION['id_tel']."', 'Cobrar nota', 'Cobrar la nota <strong>".$folio."</strong> del cliente ".$row4[0]['nombre']."', '$fechaNot', 'Ventas', '".$row1[0]['id_venta']."');");
			$error3 = $omodelo->_insertar($query3);
		}
		
		// :::::::::::::::::::::::::::::::::::::::::::::::::::::::	AGREGAR MOVIMIENTO A LA BASE DE DATOS	:::::::::::::::::::::::::::::::::::::::::::::::::::::
		$cobjeto = new controller();
		$cobjeto->_movimientos("Contado","Insertar","Se inserto un registro: ".$folio);
		//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
		
		if ($error == "si") {
			$mensaje = "Error de registro";
		}else{
			$mensaje = "Venta realizada, imprimiendo ticket";
		}
		echo $mensaje."#".$error;
	}
	...
	public function _consulta(){
		$omodelo = new m_modelo();
		extract($_POST);
		$query = ("SELECT * FROM ventas;");	// ::::::::::::::: MODIFICAR EL QUERY
		$row = $omodelo->_consultar($query);
		$numerofilas = $omodelo->numerofilas;
		if ($row == "si") {
			echo "<p class='dialogo'>Error de consulta</p>";
			 ...
			 for($i=0;$i<$numerofilas;$i++){
				...
				$detalle = "";
				$query1 = ("SELECT * FROM detalle_ventas where venta = '".$row[$i]["id_venta"]."';");
				$row1 = $omodelo->_consultar($query1);
				...
				$a[] = $detalle;							// 8
				$a[] = $row[$i]["folio"];					// 9
				$a[] = $row[$i]["descuento"];				// 10
				$a[] = $row[$i]["dias_pagar"];				// 11
			 }
		}
		foreach($a as $name){
			echo urldecode($name . "~");
