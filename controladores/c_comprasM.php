...
class comprasM {
	...
	public function _insertar(){
		...
		(null, '$fechabb', '".$_SESSION['id_tel']."', '$proveedor', '$total', '$estatus', '2', '$folio', '$dias');");
	...
	public function _reporte(){
		...
		$query = ("SELECT * FROM compras where clase = '2';");
			...
			for($i=0;$i<$numerofilas;$i++){
				...
				<div id="ventaPermisos'.$i.'" class="modal fade" tabindex="-1">
					<div class="modal-header bg-inverse bd-inverse-darken">
						...
						<h4 class="modal-title">Detalles de la compra: <strong>'.$row[$i]["nombre"].'</strong></h4>
					</div>
	...
	public function _consulta(){
		$omodelo = new m_modelo();
		extract($_POST);
		$query = ("SELECT * FROM compras where clase = '2';");	// ::::::::::::::: MODIFICAR EL QUERY
		$row = $omodelo->_consultar($query);
		$numerofilas = $omodelo->numerofilas;
		if ($row == "si") {
			echo "<p class='dialogo'>Error de consulta</p>";
