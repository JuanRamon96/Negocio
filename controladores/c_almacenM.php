	...
	public function _reporte(){
		...
		if ($row == "si") {
			...
			for($i=0;$i<$numerofilas;$i++){
				...
				$queryt = ("UPDATE productos set costo='".$total."' where id_producto = '".$row[$i]["id_producto"]."';");
				$rowt = $omodelo->_insertar($queryt);*/
				$detalle .= '
				...
				$lista2 .= '										
								<tr numeroSuma="1">
									<td class=""><a href="'.$laFoto.'" title="'.$row[$i]["nombre"].'" class="preview_fancybox"><img id="fot'.$row[$i]["id_producto"].'" alt="" style="width:50px" src="'.$laFoto.'" class=""/></a><br>'.$row[$i]["codigo"].'</td>
									<td>'.$row[$i]["nombre"].'</td>
