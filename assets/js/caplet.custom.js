   $(function() {
		...
		$(".ios-switch input:checkbox").change(function(){
			...
			if( $(this).is(':checked')){
				targetLabel.text("Activado");
			}else{
				targetLabel.text("Desactivado");
			}
		});	  
...
/*		$(".panel-tools[data-toolscolor]").each(function(i) {
		...
		$('.panel-tools .btn-reload').click(function (e) {
			...
			var datos = {"rep": pagina[1]};
			$.ajax({
                url: "index.php",
                method: "POST",
                data: datos,
				process: mientras(btn),
                success: function(data){
					recargarContenido(data);
                }
            });
		});
		function mientras(btnv){
			var  btn=btnv, panelBody=btn.closest(".panel").find(".panel-body"),
